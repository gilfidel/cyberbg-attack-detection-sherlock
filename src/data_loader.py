...
def _enum_relevant_data_files(dirpath: pathlib2.Path):
    for dfd in DATA_FILE_DESCRIPTORS.values():
        if not dfd.fields:
...
def _find_data_file(dirpath: pathlib2.Path, dfd: _DataFileDescriptor):
    try:
        fpath = next(dirpath.glob(dfd.file_name))
    except StopIteration:
        raise RuntimeError(f'{dfd} not found in {dirpath}')
